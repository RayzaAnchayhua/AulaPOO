...
public class Vendedor {
    ...
    //inserir produtos
    public void inserir(Produto produto) {this.listaProdutos.add(produto);}

    public void listar(){
        if (!this.listaProdutos.isEmpty()) {  // Empty = vazio
            for (int i = 0; i < this.listaProdutos.size(); i++) {
                System.out.println("Item: " + (i+1) + " - Produto: " + this.listaProdutos.get(i).getNome());
            }
        } else {
